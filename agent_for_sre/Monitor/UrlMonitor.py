						...
						try:
							...
							if(str(self.monitor_item['return_content'])!=''):
								if(buffer.getvalue().decode('utf-8').replace('\n', '')==self.monitor_item['return_content']):
									...
									self.result.append({'ip':self.ip,'url':url,'msg':str(self.monitor_item['chname'])+buffer.getvalue().decode('utf-8').replace('\n', '')+self.monitor_item['return_content']+'ERROR','error':1})
